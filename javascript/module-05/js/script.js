...
const getAllNames = arr => {
  const usersName = arr.map(user => user.name);
  return usersName;
};
...
const getInactiveUsers = arr => {
  const inactiveUsers = arr.filter(user => !user.isActive);
...
const getUsersWithAge = (arr, min, max) => {
  usersWithAge = arr.filter(user => user.age >= min && user.age < max);
  return usersWithAge;
};
...
const getTotalBalance = arr => {
  const usersTotalBalance = arr.reduce((acc, user) => acc + user.balance, 0);
...
const getUsersByFriend = (arr, name) => {
  const friendName = arr.reduce(
    (acc, user) => user.friends.includes(name) ? acc.concat(user.name) : acc,
    []);
...
const getUserNamesSortedByFriendsCount = arr => {
  const userNamesSortedByFriendsCount = arr
    .sort((a, b) => a.friends.length - b.friends.length)
    .map (user => user.name);
  return userNamesSortedByFriendsCount;
};
